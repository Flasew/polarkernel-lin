...
class Gih(object):
    ...
    def __init__(self, irq = -1, delayTime = -1, wrtSize = -1, path = ''):
        ...
        if delayTime != -1:
            configureDelayTime(delayTime)

        self.wrtSize = wrtSize
        if wrtSize != -1: 
            configureWrtSize(wrtSize)
    ...
    def configureIRQ(self, irq):
        ...
        if gih_config.configure_irq(fd, irq) == irq:
            self.irq = irq
        else:
            self.irq = -1
    ...
    def configureDelayTime(self, delayTime):
        ...
        if gih_config.configure_sleep_t(fd, delayTime) == delayTime:
            self.delayTime = delayTime
        else:
            self.delayTime = -1
    ...
    def configureWrtSize(self, wrtSize):
        ...
        if gih_config.configure_wrt_sz(fd, wrtSize) == wrtSize:
            self.wrtSize = wrtSize
        else:
            self.wrtSize = -1
    ...
    def configurePath(self, path):
        ...
        if gih_config.configure_path(fd, path) == len(path):
            self.path = path
            return True
        else:
            self.path = ''
    ...
    def configureFinish(self):
        ...
        return (gih_config.configure_finish(fd) == 0)
